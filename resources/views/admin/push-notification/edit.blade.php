...
@section('content')
    <div class="container-fluid">
        ...
        <form action="{{ route('admin.push-notifications.update', $notification) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row">
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="title" name="title" required
                                    value="{{ old('title', $notification->title) }}" placeholder="{{ translate('messages.Enter a concise title') }}">
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" id="message" name="message" rows="3" required
                                    placeholder="{{ translate('messages.Enter the notification message') }}">{{ old('message', $notification->message) }}</textarea>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" id="type" name="type">
                                    <option value="announcement" {{ old('type', $notification->type) == 'announcement' ? 'selected' : '' }}>{{ translate('messages.Announcement') }}</option>
                                    <option value="price_alert" {{ old('type', $notification->type) == 'price_alert' ? 'selected' : '' }}>{{ translate('messages.Price Alert') }}</option>
                                    <option value="promotion" {{ old('type', $notification->type) == 'promotion' ? 'selected' : '' }}>{{ translate('messages.Promotion') }}</option>
                                    <option value="system" {{ old('type', $notification->type) == 'system' ? 'selected' : '' }}>{{ translate('messages.System Update') }}</option>
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="link_url" name="link_url" 
                                    value="{{ old('link_url', $notification->link_url) }}" placeholder="{{ translate('messages.Enter URL or ID') }}">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" id="zone_id" name="zone_id">
                                    ...
                                    @foreach($zones ?? [] as $zone)
                                        <option value="{{ $zone->id }}" {{ old('zone_id', $notification->zone_id) == $zone->id ? 'selected' : '' }}>{{ $zone->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-control select2" id="target_audience" name="target_audience" required>
                                    <option value="all" {{ old('target_audience', $notification->target_audience) == 'all' ? 'selected' : '' }}>{{ translate('messages.All Users') }}</option>
                                    <option value="volunteers" {{ old('target_audience', $notification->target_audience) == 'volunteers' ? 'selected' : '' }}>{{ translate('messages.Volunteers') }}</option>
                                    <option value="inactive" {{ old('target_audience', $notification->target_audience) == 'inactive' ? 'selected' : '' }}>{{ translate('messages.Inactive Users (30+ days)') }}</option>
                                    <option value="new" {{ old('target_audience', $notification->target_audience) == 'new' ? 'selected' : '' }}>{{ translate('messages.New Users (< 7 days)') }}</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            @if($notification->image)
                                <div class="mb-3">
                                    <p class="small text-muted">{{ translate('messages.Current image') }}:</p>
                                    <img src="{{ asset('storage/' . $notification->image) }}" alt="{{ translate('messages.Current image') }}" class="img-fluid rounded" style="max-height: 200px;">
                                </div>
                            @endif
                            <div class="image-preview-container mb-3" onclick="document.getElementById('notificationImage').click();">
                                <div class="image-preview" id="imagePreview">
                                    <i class="fas fa-camera"></i>
                                    <span>{{ translate('messages.Click to Upload New Image') }}</span>
                                </div>
                            </div>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <div class="notification-preview p-3 bg-light rounded">
                                ...
                                <h6 class="mb-1 font-weight-bold" id="preview-title">{{ $notification->title }}</h6>
                                <p class="mb-2 small" id="preview-message">{{ $notification->message }}</p>
                                @if($notification->image)
                                    <div id="preview-image-container">
                                        <img id="preview-image" src="{{ asset('storage/' . $notification->image) }}" alt="{{ translate('messages.Notification Image') }}" class="img-fluid rounded mb-1" style="max-height: 100px; width: 100%; object-fit: cover;">
                                    </div>
                                @else
                            ...
                            <div class="d-grid gap-2 mt-4">
                                <button type="submit" class="btn btn-primary btn-block">
                                    <i class="fas fa-save mr-1"></i> {{ translate('messages.Save Changes') }}
                                </button>
                            </div>
                        </div>
                    </div>
